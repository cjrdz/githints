...
//
// The threat model is an attacker who can write to store.db but does not
// know the integrity key. With HMAC chaining they cannot insert, delete, or
// modify rows without breaking the chain. The key is derived from a
// machine-local salt plus the repo's git user.email, so it is not portable
// across machines but is stable for normal use on one machine.
package integrity
...
// SaltPath returns the path to the salt file under the repo root.
func SaltPath(root string) string {
	return filepath.Join(root, ".githints", saltFileName)
}

// LoadOrCreateSalt reads the per-machine salt from .githints/.salt,
// creating it with 0600 permissions if it does not yet exist.
