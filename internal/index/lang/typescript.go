...
//   - Package specifiers ("svelte", "astro") never resolve to repo files.
//     tsconfig path aliases ("@core/x") resolve through the active
//     TSPathsConfig when the repo has a tsconfig.json/jsconfig.json with
//     compilerOptions.paths; only the first target of each pattern is used.
