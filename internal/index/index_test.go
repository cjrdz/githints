...
func TestFullScanResolvesTSConfigPathsAndPrunes(t *testing.T) {
	st, dir := tempStore(t)
	defer st.Close()
	...
	// Barrel file: re-export only, so it produces imports but zero symbols.
	write("src/lib/util/index.ts", `export * from "./format";
`)
	write("src/lib/util/format.ts", "export function format(v: string): string {\n  return v;\n}\n")
	// Consumer imports the barrel through the tsconfig alias.
	write("src/features/catalog/page.ts", `import { format } from "@lib/util";
import { helper } from "./helper";

export const title = format(helper("x"));
`)
	write("src/features/catalog/helper.ts", "export function helper(v: string): string {\n  return v;\n}\n")
	// Page with frontmatter imports only (no declarations).
...
`)
	...
	// Plant a stale note from a "deleted" source before the scan; the full
	// scan must prune it along with its now-empty directory.
	staleNote := lang.NotePath(root, "src/old/deleted.ts")
	if err := os.MkdirAll(filepath.Dir(staleNote), 0o755); err != nil {
		t.Fatalf("mkdir stale: %v", err)
	}
	if err := os.WriteFile(staleNote, []byte("# src/old/deleted.ts\n"), 0o644); err != nil {
		t.Fatalf("write stale note: %v", err)
	}

	if err := FullScan(st, lang.ScanOptions{Root: root, Languages: []string{"typescript", "astro"}, MaxFileSize: 1024, ParseTimeout: 5 * time.Second}, false, 0); err != nil {
		t.Fatalf("FullScan: %v", err)
	}
	...
	// The alias import resolves through tsconfig paths to the barrel file,
	// so the barrel gets a note despite having no symbols, and lists its
	// dependent.
	barrel := read("src/lib/util/index.ts")
	if !strings.Contains(barrel, "## Imported by") || !strings.Contains(barrel, "page.ts") {
		t.Errorf("barrel note missing Imported by via alias resolution:\n%s", barrel)
	}

	// The consumer's note has an outbound Imports section that links the
	// alias to the barrel note and the relative import to the helper note.
	page := read("src/features/catalog/page.ts")
	if !strings.Contains(page, "## Imports") {
		t.Fatalf("consumer note missing Imports section:\n%s", page)
	}
	if !strings.Contains(page, "[src/lib/util](../../lib/util/index.ts.md)") {
		t.Errorf("alias import not linked to barrel note:\n%s", page)
	}
	if !strings.Contains(page, "[src/features/catalog/helper](helper.ts.md)") {
		t.Errorf("relative import not linked to helper note:\n%s", page)
	}

	// Import-only Astro pages get notes too.
	about := read("src/pages/about.astro")
	if !strings.Contains(about, "## Imports") || !strings.Contains(about, "Base.astro.md") {
		t.Errorf("astro page note missing linked import:\n%s", about)
	}

	// The stale note is gone, and its emptied directory was removed.
	if _, err := os.Stat(staleNote); !os.IsNotExist(err) {
		t.Errorf("stale note not pruned: %v", err)
	}
	if _, err := os.Stat(filepath.Dir(staleNote)); !os.IsNotExist(err) {
		t.Errorf("empty stale note directory not pruned: %v", err)
	}
}
...
func TestVerifyIndexReportsDriftAndCoverage(t *testing.T) {
	st, dir := tempStore(t)
	defer st.Close()
	...
	// Plant drift AFTER the scan: a stale note and a ghost row.
	staleNote := lang.NotePath(root, "old.go")
	if err := os.MkdirAll(filepath.Dir(staleNote), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(staleNote, []byte("# old.go\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := st.InsertSymbols([]lang.Symbol{{Name: "Ghost", Kind: lang.KindFunc, FilePath: "ghost.go", LineStart: 1, LineEnd: 2}}); err != nil {
		t.Fatalf("insert ghost rows: %v", err)
	}

	report, err := VerifyIndex(st, opts)
	if err != nil {
		t.Fatalf("VerifyIndex: %v", err)
	}
	if !report.Drift() {
		t.Error("expected drift from planted stale note + ghost rows")
	}
	if len(report.StaleNotes) != 1 || report.StaleNotes[0] != "old.go" {
		t.Errorf("StaleNotes = %v", report.StaleNotes)
	}
	if len(report.GhostFiles) != 1 || report.GhostFiles[0] != "ghost.go" {
		t.Errorf("GhostFiles = %v", report.GhostFiles)
	}

	// ignored.go is gitignored: not indexed, reported with its reason.
	found := false
	for _, u := range report.Uncovered {
		if u.Path == "ignored.go" {
			found = true
			if u.Reason != "gitignored" {
				t.Errorf("ignored.go reason = %q, want gitignored", u.Reason)
			}
		}
	}
	if !found {
		t.Errorf("ignored.go missing from Uncovered: %v", report.Uncovered)
	}

	// The full scan also seeded the Obsidian graph preset.
	presetPath := filepath.Join(root, ".githints", ".obsidian", "graph.json")
	data, err := os.ReadFile(presetPath)
	if err != nil {
		t.Fatalf("graph preset not written: %v", err)
	}
	if !strings.Contains(string(data), "path:index") {
		t.Errorf("preset missing path:index filter: %s", data)
	}

	// A second scan must not clobber user-modified preset content.
	custom := `{"search":"path:index/src","hideUnresolved":false}`
	if err := os.WriteFile(presetPath, []byte(custom), 0o644); err != nil {
		t.Fatal(err)
	}
