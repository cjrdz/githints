...
// gitignoreManagedStart/End bracket the block githints owns in the repo
// root .gitignore. Re-running init updates the block in place.
const (
...
func ensureGitignore(root string, share bool) error {
	...
	// Trim trailing blank lines so re-runs don't add infinite whitespace.
	for len(out) > 0 && out[len(out)-1] == "" && (len(out) == 1 || out[len(out)-2] == "") {
		out = out[:len(out)-1]
	}
...
func cmdServe() error {
	...
	return mcpserver.Run(root, st, cfg, version)
}
...
// cmdRender re-generates every per-file hint and the root CHANGES.md from the
...
func cmdRender() error {
	...
	if err != nil {
		return fmt.Errorf("list files with history: %w", err)
	}
	for _, f := range files {
		if err := hint.RenderFile(st, root, f, recorder.HistoryLimitPerFile); err != nil {
			return fmt.Errorf("render file hint for %s: %w", f, err)
...
// cmdIndexVerify reports drift between the index, the rendered notes, and
// the tracked source files: stale notes, ghost rows, and covered-language
// files with no index rows (with reasons). It exits non-zero on real drift
// (stale notes or ghosts); uncovered files are informational.
func cmdIndexVerify(args []string) error {
	...
	report, err := index.VerifyIndex(db, lang.ScanOptions{
		Root:         root,
		Languages:    cfg.Index.Languages,
		MaxFileSize:  int64(cfg.Index.MaxFileSize),
		ParseTimeout: time.Duration(cfg.Index.ParseTimeoutMS) * time.Millisecond,
	...
	fmt.Printf("notes on disk: %d\n", report.Notes)
	if len(report.StaleNotes) > 0 {
		fmt.Printf("stale notes (source not indexed): %d\n", len(report.StaleNotes))
		for _, s := range report.StaleNotes {
			fmt.Printf("  - %s\n", s)
		}
	}
	if len(report.GhostFiles) > 0 {
		fmt.Printf("ghost rows (file deleted from disk): %d\n", len(report.GhostFiles))
	...
	if !report.Drift() && len(report.Uncovered) == 0 {
		fmt.Println("index is in sync with the repository")
	} else if !report.Drift() {
		fmt.Println("no drift (run githints index to refresh; uncovered files above are skipped by design)")
	}

	if report.Drift() {
		return fmt.Errorf("index drift detected; run githints index to repair")
	}
	return nil
}
